...
\begin{enumerate}
	...
	\[
	\bar{f}_{ac}(x) = \bar{f}_a(x) - \sum_{i = 1}^{n}\bar{f}_a(x_i).
	\]
	\item Para cada valor observado de $X$, plotar $\bar{f}_{ac}(x) \times x$. 
...
\begin{itemize}
	...
	\item Ao contrário do gráfico de dependência parcial e do gráfico de esperança condicional individual, que apresentam no eixo $y$ o valor predito, o gráfico de efeitos locais acumulados consideram nesse eixo o efeito estimado do preditor $X$ na variável resposta.
\end{itemize}
...
Uma outra técnica utilizada para interpretar modelos caixa-preta é o LIME (\textit{Local snterpretable Model-Agnostic Explanations}) \citep{Ribeiro2016}. Dado um modelo não-interpretável, essa técnica consiste em entender por que o modelo fez uma certa predição, explicando quais preditores foram responsáveis por ela. Nos últimos anos, o LIME vem sendo muito utilizado para explicar, corrigir e aperfeiçoar modelos não-interpretáveis.
