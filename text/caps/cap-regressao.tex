...
Modelos não-supervisionados são aqueles que não consideram uma variável resposta. Geralmente são utilizados para criar agrupamentos e fazer redução de dimensionalidade. Em estudos de poluição do ar, modelos não-supervisionados são muito utilizados para detecção de fontes de poluentes. A seguir, discutiremos os modelos usualmente utilizados.
...
Imagine que em um estudo estamos investigando a concentração de dois poluentes, digamos $X_1$ e $X_2$. Dada uma amostra de tamanho $n$ dessas variáveis, se quiséssemos explorar esses dados descritivamente, poderíamos construir um gráfico de dispersão de $X_1$ contra $X_2$ e, a partir dele, observar a variabilidade desses poluentes e também como eles estão correlacionados. Se eles apresentarem correlação positiva, teríamos indícios de que eles são gerados pela mesma fonte ou pelas mesmas condições atmosféricas.

Imagine agora que, em vez de 2 poluentes, tivéssemos 10. Se quiséssemos construir todos os gráficos de dispersão para as combinações dois a dois, precisaríamos analisar 45 gráficos, sendo que cada um deles só traria uma pequena parte da informação contida nos dados.

Em um contexto geral, para $p$ poluentes, gostaríamos de uma maneira de visualizar o máximo possível da informação contida nas $p$ dimensões geradas pelos preditores $X_1, \dots, X_p$ em uma representação com poucas dimensões. Esse é exatamente o objetivo da análise de componentes principais.
...
\]
Isso implica que, em geral, com apenas as primeiras variáveis $Z$'s, digamos $Z_1$ e $Z_2$, podemos explicar a maior parte da variabilidade dos preditores $X$'s. Assim, $Z_1$ e $Z_2$ representariam em apenas 2 dimensões a maior parte da informação contida nos dados.

Cada variável $Z_i$, chamada de $i$-ésima componente principal, é uma combinação linear dos preditores $X_1, X_2, \dots, X_p$ dada por
...
Voltando ao nosso exemplo com os poluentes, se a análise de componentes principais indicasse os poluentes $X_1$, $X_3$ e $X_5$ como aqueles como maiores pesos para o componente $Z_1$ e os poluentes $X_2$, $X_3$ e $X_4$ como aqueles com maior peso para o componente $Z_2$, então esses dois grupos de poluentes caracterizariam duas fontes não-correlacionadas de poluição\footnote{ A identificação da fonte deve então ser feita a partir dos poluentes que a compõe utilizando conhecimento teórico sobre emissões ou resultados de estudo anteriores.}.

Os cálculos por trás da análise de componentes principais envolvem decomposição espectral \citep{Nicholson2006}, uma técnica de álgebra linear para representar matrizes em função de seus autovetores e autovalores. 
...
\subsection{Positive Matrix Factorization}
