...
cria_formula <- function(mort) {
  ...
  padrao <- "share_gas + hm + tp + trend + dayofweek + dv_workday + week"
...
cria_receita <- function(df_model, formula) {
  df_model %>%
    ...
    step_dummy(dayofweek, week, one_hot = TRUE)
}

vars <- c("n_mortes_idosos")
...
p_pdp + p_ale
ggsave(
  filename = "text/figuras/cap-mort-rf-graficos-iml.pdf", 
  width = 7, height = 5
)
